...
public class ReservationTableUI extends JFrame {
    ...
    /** เก็บ checkbox ของแต่ละชั้น (คีย์: ชื่อชั้น → ลิสต์เช็คบ็อกซ์ของกริดชั้นนั้น) */
    private final Map<String, java.util.List<JCheckBox>> floorCheckBoxes = new HashMap<>();
    /** แพแนลหลักวางกริดหลายชั้น (ล้าง/สร้างใหม่ตอนรีเฟรช) */
    ...
    /** รายการช่วงเวลาที่เปิดให้จอง (แก้/เพิ่มตรงนี้ที่เดียว) */
    ...
    public ReservationTableUI() {
        ...
        cmbDate = new JComboBox<>(new String[]{ today.format(DDMMYYYY), tomorrow.format(DDMMYYYY) });
        ...
        cmbFloor = new JComboBox<>(new String[]{"ทุกชั้น", "ชั้น 1", "ชั้น 2"});
        ...
        btnReserve.addActionListener((ActionEvent e) -> {
            ...
            String username = Model.UserSession.getCurrentUsername();

            // เช็คโควตา/วัน
            int used = countUserActiveSlotsUI(username, date);
            int remaining = MAX_SLOTS_PER_USER_PER_DAY - used;
            if (remaining <= 0) {
            ...
            int pickedThisClick = 0;

            // วนทุกเช็คบ็อกซ์ที่เลือก
        ...
        // ===== รับฟังเหตุการณ์จาก ReservationManager (อัปเดต UI ตามข้อมูลจริง) =====
    ...
    private void addFloorTable(JPanel parent, String floorName, String[] rooms, String[] times, boolean limitTo3) {
        ...
        JPanel grid = new JPanel(new GridLayout(maxRooms+1, times.length+1, 1, 1));
        ...
        for (int i = 0; i < maxRooms; i++) {
            String room = rooms[i];
            grid.add(makeCell(room, Color.LIGHT_GRAY, true)); // ชื่อห้อง
